...
class influxdb::config {
  #
  # These settings are common to all versions
  #
  ...
  if $influxdb::version =~ /0\.8\.(\d+)/ {
    #
    # These settings are specific to 0.8.*
    #
    ...
    #
    # These settings are specific to 0.9.* (and beyond presumably)
    #
    ...
    ini_setting { 'initialization_join_urls':
      ...
      setting => 'join-urls',
      value   => "\"$influxdb::initialization_join_urls\"",
    }
    ...
    ini_setting { 'authentication_enabled':
      ...
      setting => 'enabled',
      value   => $influxdb::authentication_enabled,
    }

    # [broker]
    ini_setting { 'broker_directory':
      section => 'broker',
      setting => 'dir',
    ...
    # [cluster]
    ini_setting { 'cluster_state_storage':
      section => 'cluster',
      setting => 'dir',
      value   => "\"$influxdb::cluster_state_storage_dir\"",
    }
  }
